...
vector<pair<string, int> > CBNET :: GetFriends( )
{
	...
	for( vector<CIncomingFriendList *> :: iterator i = m_Friends.begin( ); i != m_Friends.end( ); i++ )
	{
		...
		result.push_back(pair<string, int>((*i)->GetLocation( ), (*i)->GetArea( ) ) );
	}
...
vector<pair<string, int> > CBNET :: GetClan( )
{
	...
	for( vector<CIncomingClanList *> :: iterator i = m_Clans.begin( ); i != m_Clans.end( ) && k < 50; i++, k++ )
	{
		...
		result.push_back(pair<string, int>((*i)->GetRank( ), (*i)->GetRawStatus( ) ) );
	}
...
void CBNET :: ProcessChatEvent( CIncomingChatEvent *chatEvent )
{
	...
	if( Event == CBNETProtocol :: EID_SHOWUSER )
		CONSOLE_AddChannelUser( User, UserFlags );
	else if( Event == CBNETProtocol :: EID_USERFLAGS )
		CONSOLE_UpdateChannelUser( User, UserFlags );
	...
	{
		CONSOLE_Print( "[BNET] joined channel [" + Message + "]", false );
		...
		CONSOLE_Print( "[BNET] channel does not exist", false  );
	else if( Event == CBNETProtocol :: EID_CHANNELRESTRICTED )
		CONSOLE_Print( "[BNET] channel restricted", false  );
	else if( Event == CBNETProtocol :: EID_ERROR )
		CONSOLE_Print( "[ERROR] " + Message, false );
	else if( Event == CBNETProtocol :: EID_EMOTE )
		CONSOLE_Print( "[EMOTE] <" + User + "> " + Message, false  );
	else if( Event == CBNETProtocol :: EID_INFO )
		CONSOLE_Print( "[INFO] " + Message, false );
}
	...
	if( m_LoggedIn )
	{
		...
		else
		{
			...
			if ( whisper )
			{
				...
				{
					CONSOLE_Print( "[WHISPERED: Friends] " + chatCommand.substr( r, chatCommand.length( ) ) , false );
					m_GProxy->SendLocalChat( "Whispered to friends: " + chatCommand.substr( r, chatCommand.length( ) ) );
				}
			}
			else if (!hidden) CONSOLE_Print( "<" + m_UserName + "> " + chatCommand, false );

...
void CCurses :: SetGProxy( CGProxy* nGProxy )
{
	...
	Print( "", true );
	Print( "  GProxy++ connects to battle.net and looks for games for you to join.", true );
	Print( "  If GProxy++ finds any they will be listed on the Warcraft III LAN screen.", true );
	Print( "  To join a game, simply open Warcraft III and wait at the LAN screen.", true );
	Print( "  Standard games will be white and GProxy++ enabled games will be blue.", true );
	Print( "  Note: You must type \"/public\" to enable listing of public games.", true );
	Print( "", true );
	Print( "  If you want to join a specific game, type \"/game <gamename>\".", true );
	Print( "  GProxy++ will look for that game for up to two minutes before giving up.", true );
	Print( "  This is useful for private games.", true );
	Print( "", true );
	Print( "  Please note:", true );
	Print( "  GProxy++ will join the game using your battle.net name, not your LAN name.", true );
	Print( "  Other players will see your battle.net name even if you choose another name.", true );
	Print( "  This is done so that you can be automatically spoof checked.", true );
	Print( "", true );
	Print( "  Also note that other players will not see whispers you receive.", true );
	Print( "", true );
	...
	Print( "", true );
	Print( "{ CursesMod - User Interface }", true );
	Print( " - Arrow keys: tabs, message history", true );
	Print( " - Mouse: tabs, scrolling", true );
	Print( " - CTRL-C: Copy text, CTRL-V: Paste text", true );
	Print( " - Page Up, Page Down: scrolling", true );
	Print( "", true );
}
